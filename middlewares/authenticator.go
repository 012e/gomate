...
func CookieAuthenticator(c *controllers.DefaultController) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if err != nil || strings.TrimSpace(cookie) == "" {
			logrus.Infof("failed with status %e", err)
		...
		if errors.Is(gorm.ErrRecordNotFound, err) {
			ctx.AbortWithStatusJSON(http.StatusUnauthorized, json.Fail("not logged in"))
			return
		} else if err != nil {
			ctx.AbortWithStatusJSON(http.StatusUnauthorized, json.FailUnknown())
			return
		}
